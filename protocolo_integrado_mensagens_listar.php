...
try {
  ...
  $bolAcaoConfigurarPublicacao = SessaoSEI::getInstance()->verificarPermissao('protocolo_integrado_configurar_publicacao');
  ...
  if ($bolAcaoConfigurarPublicacao){
    ...
    $strLinkConfigurarHistorico = SessaoSEI::getInstance()->assinarLink('controlador.php?acao=protocolo_integrado_configurar_publicacao&acao_origem='.$_GET['acao']);
  }
  ...
  if ($numRegistros > 0){
    ...
    if ($_GET['acao']=='protocolo_integrado_mensagens_listar'){
      $bolAcaoReativar = false;
      $bolAcaoConsultar = SessaoSEI::getInstance()->verificarPermissao('protocolo_integrado_mensagens_listar');
      $bolAcaoAlterar = SessaoSEI::getInstance()->verificarPermissao('protocolo_integrado_mensagens_alterar');
    ...
    for($i = 0;$i < $numRegistros; $i++){
	  ...
	  $strResultado .= '<td align="center">';
	      ...
	      if ($arrObjProtocoloIntegradoDTO[$i]->getStrSinPublicar()=='S'){
	        $strResultado .= '<a class="enabled" id="linkEditarMensagem_'.$i.'" href="'.PaginaSEI::getInstance()->formatarXHTML(SessaoSEI::getInstance()->assinarLink('controlador.php?acao=protocolo_integrado_mensagens_alterar&acao_origem='.$_GET['acao'].'&acao_retorno='.$_GET['acao'].'&id_mensagem_protocolo_integrado='.$arrObjProtocoloIntegradoDTO[$i]->getNumIdProtocoloIntegrado())).'" tabindex="'.PaginaSEI::getInstance()->getProxTabTabela().'"><img src="'.PaginaSEI::getInstance()->getDiretorioImagensGlobal().'/alterar.gif" title="Alterar Tipo de Andamento" alt="Alterar Tipo de Andamento" class="infraImg" /></a>&nbsp;';
	      }else {
	      	
			 $strResultado .= '<a class="disabled" id="linkEditarMensagem_'.$i.'" href="'.PaginaSEI::getInstance()->formatarXHTML(SessaoSEI::getInstance()->assinarLink('controlador.php?acao=protocolo_integrado_mensagens_alterar&acao_origem='.$_GET['acao'].'&acao_retorno='.$_GET['acao'].'&id_mensagem_protocolo_integrado='.$arrObjProtocoloIntegradoDTO[$i]->getNumIdProtocoloIntegrado())).'" tabindex="'.PaginaSEI::getInstance()->getProxTabTabela().'"><img src="'.PaginaSEI::getInstance()->getDiretorioImagensGlobal().'/alterar.gif" title="Alterar Tipo de Andamento" alt="Alterar Tipo de Andamento" class="infraImg" /></a>&nbsp;';
