...
class ProtocoloIntegradoClienteWS extends SoapClient {
	...
	private function validarConexaoWebService() {
		...
		if ($curl_errno) {
			$e =  new Exception($header."Requisi��o CURL resultou no seguinte erro: " . $curl_error . "(C�digo: " . $curl_errno . ")");
			if ($curl_errno == 60) {
			    throw new InfraException("Certificado inv�lido ou ausente.",$e);
			} else {
				throw new InfraException("Ocorreu um problema ao realizar a conex�o ao Web Service do Protocolo Integrado. Acesse o log do SEI para maiores detalhes", $e);
			}
		} else {
			if ($httpCode!=200) {
				if (strlen($header)>0) {
					...
					$e = new Exception("503 Service Unavailable.N�o foi poss�vel conectar ao servidor");
				}
				throw new InfraException("Ocorreu um problema ao realizar a conex�o ao Web Service do Protocolo Integrado. Acesse o log do SEI para maiores detalhes.", $e);
			}
		}
