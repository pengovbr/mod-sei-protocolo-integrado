...
class ProtocoloIntegradoRN extends InfraRN {
  ...
  protected function listarConectado(ProtocoloIntegradoDTO $objProtocoloDTO) {
    try {
      ...
      //Valida Permissao
      SessaoSEI::getInstance()->validarAuditarPermissao('md_pi_mensagens_listar', __METHOD__, $objProtocoloDTO);
  ...
  protected function alterarControlado(ProtocoloIntegradoDTO $protocoloIntegradoDTO){
    try {
      ...
      //Valida Permissao
      SessaoSEI::getInstance()->validarAuditarPermissao('md_pi_mensagens_alterar', __METHOD__, $protocoloIntegradoDTO);
  ...
  protected function alterarOperacoesPublicacaoControlado(ProtocoloIntegradoDTO $protocoloIntegradoDTO){
    try {
      ...
      //Valida Permissao
      SessaoSEI::getInstance()->validarAuditarPermissao('md_pi_configurar_publicacao', __METHOD__, $protocoloIntegradoDTO);
    ...
    for($k=0; $k<count($arrObjAtributoAndamentoDTO); $k++){
      ...
      switch($objAtributoAndamentoDTO->getStrNome()){
        ...
        case 'NIVEL_ACESSO':
                ...
                $idNivelAcesso = $objAtributoAndamentoDTO->getStrIdOrigem();
                if (isset($arrObjNivelAcessoDTO[$idNivelAcesso])) {
                    $strNomeTarefa = str_replace('@NIVEL_ACESSO@', $arrObjNivelAcessoDTO[$idNivelAcesso]->getStrDescricao(), $strNomeTarefa);
                } else {
