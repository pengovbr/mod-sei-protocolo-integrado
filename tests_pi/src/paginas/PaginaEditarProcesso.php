...
class PaginaEditarProcesso extends PaginaTeste
{
  ...
  public function selAssunto($classificacaoAssunto)
    {
    try {
      try{
        $input = $this->test->byId("selAssunto");
        $this->test->select($input)->byXPath(".//option[contains(text(), '{$classificacaoAssunto}')]")->click();
      }
      catch (Exception $e){
          $this->test->byId("txtAssunto")->click();
          $this->test->byId('txtAssunto')->value($classificacaoAssunto);
          sleep(1);
          $this->test->byPartialLinkText($classificacaoAssunto)->click();
      }
    }
    catch (Exception $e){
