	  ...
	  if (isset($_REQUEST['id_mensagem_protocolo_integrado'])){
			...
			if(isset($_POST['hdnFomularioSubmetido']) ){
				...
				if($objRetornoProtocoloIntegradoDTO->getStrSinPublicar()=='S'){
					...
					header('Location: '.SessaoSEI::getInstance()->assinarLink('controlador.php?acao=protocolo_integrado_mensagens_listar'));
          			die;
...
?>
  ...
  <button type="button" accesskey="F" id="btnFecharSelecao" value="Fechar" onclick="location.href='<?=SessaoSEI::getInstance()->assinarLink('controlador.php?acao=protocolo_integrado_mensagens_listar')?>';" class="infraButton"><span class="infraTeclaAtalho">C</span>ancelar</button>
  <?
